 ...
 *
 */
public class CargoDepositS implements RectangleVerifier, RectangleAndColorVerifier {
...
// switch-case behaviour attributes
    ...
    private void flowScript() throws AWTException, InterruptedException, IOException, TesseractException {
        ...
        switch (walkThrough) {
            ...
            case 2 -> {
                if ((isStaticClicker && staticCheck()) || this.dynamicCheck()) {
                    walkThrough++;
                } else {
                    clickEvents.dragScreen();
                }
    ...
    private boolean isClickerDeprecated(String clicker) {
        return (boolean) jsonLogs.readClicker(clicker).get("deprecated");
    }
    ...
    private boolean staticCheck() throws IOException, TesseractException, AWTException, InterruptedException {
        ...
        if (isClickerDeprecated("undock")) {
            ...
            if (rectangleVerifier(undockButton, "DYNAMICALLY IDENTIFIED UNDOCK BUTTON", LEFTCLICK)) {
                saveClicker(undockButton, "undock");
                return true;
            }

        } else {
            rectangleVerifier(this.getClicker("undock"), "STATIC CLICKER UNDOCK BUTTON", LEFTCLICK);
            return true;
        }
        return false;
    }

    private boolean dynamicCheck() throws IOException, TesseractException, AWTException, InterruptedException {
        Rectangle undockButton = segmentedRegions.getRectangle(resolution.getUndockButtonList(), resolution.getUndockDeadZoneTuple());
